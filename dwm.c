...
static void updatesystray(int updatebars);
...
drawbar(Monitor *m)
{
	int x, w, sw = 0, stw = 0, etwl = 0, etwr = 0, n = 0, scm;
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (!m->extrabarwin) {
            w = m->ww;
			m->extrabarwin = XCreateWindow(dpy, root, m->wx, m->eby, w, bh, 0, depth,
				InputOutput, visual,
				CWOverrideRedirect|CWBackPixel|CWBorderPixel|CWColormap|CWEventMask, &wa);
